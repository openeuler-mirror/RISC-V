# openEuler RISC-V SIG 文档


## [新手必看](tutorials)
- [拥有账户](tutorials/README.md)
- [工作流程](tutorials/workflow-for-build-a-package.md)

## 开发环境
- [QEMU-riscv64 版 openEuler 的获取和运行](tutorials/vm-qemu-oErv.md)
- [基于 systemd-nspawn 和 QEMU User Mode 搭建 openEuler RISC-V 软件包的快速开发环境](tutorials/qemu-user-mode.md)
- [软件源简要文档](tutorials/rpm-repository.md)

## 外部文档
- [**openEuler 官方打包规范**](https://gitee.com/openeuler/community/blob/master/zh/contributors/packaging.md)
- [zxs-un整理的文档](https://gitee.com/zxs-un/doc-port2riscv64-openEuler)
- [lvxiaoqian整理的文档](https://gitee.com/lvxiaoqian/memo)